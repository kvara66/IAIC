...
- action_loss: mean of 6 per-dim MAE in normalized space

Action extractor: placeholder using submission_kit ckpt.
TODO: swap to teammate's independently trained weights before final training.
"""
...
TEAMMATE_CKPT  = Path(__file__).parent.parent / "action" / "best_slim.pt"
TEAMMATE_CODE  = Path(__file__).parent.parent / "action"
...
class DINOLoss(nn.Module):
    """1 - cosine_similarity of vit_small_patch14_dinov2 CLS tokens."""
...
class R3DLoss(nn.Module):
    """1 - cosine_similarity of R3D-18 video features (Stage 2 only)."""
...
class ActionMAELoss(nn.Module):
    """
    Per-dim MAE between action extractor output and GT normalized actions.
    Returns (total_loss, dict of per-dim losses).

    TODO: Replace submission_kit ckpt with teammate's independently trained weights
          before actual training run.
    """

    def __init__(self, ckpt_path: str | None = None, device: torch.device = torch.device("cpu")):
        super().__init__()
        if ckpt_path is None:
            ckpt_path = str(TEAMMATE_CKPT)

        sys.path.insert(0, str(TEAMMATE_CODE))
        from action_extractor import load_action_extractor
        self.extractor = load_action_extractor(ckpt_path, device="cpu")
        self.extractor.to(device).requires_grad_(False)
        # cuDNN RNN backward requires training mode even when params are frozen
    ...
    def forward(
        ...
        x = pred_video.permute(0, 2, 1, 3, 4)
