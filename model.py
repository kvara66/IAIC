...
Pipeline:
  ...
  [state_i, state_i1] (B,12) → ActionMLP → cond (B,256)
...
class VAEWrapper(nn.Module):
    def __init__(self, pretrained: str = "stabilityai/sdxl-vae"):
        super().__init__()
    ...
    def decode(self, z: torch.Tensor) -> torch.Tensor:
        ...
        return checkpoint(lambda x: self.vae.decode(x).sample, z / self.scale, use_reentrant=False)
...
class LatentResidualUNet(nn.Module):
    ...
    Channels: 4 → 64 → 128 → 256 (mid) → 128 → 64 → 4
    """
