"""
Latent Residual UNet with Action FiLM Conditioning (Non-AR, bigger UNet).

Pipeline:
  frame_0 (B,3,H,W) → SDXL VAE encode → latent_0 (B,4,60,80)  [always frame_0]
  [state_0, state_i] (B,12) → ActionMLP → cond (B,512)          [absolute positions]
...
class ActionMLP(nn.Module):
    """[state_0 (6D), state_i (6D)] → cond vector (cond_dim)"""
    ...
    def forward(self, state_0: torch.Tensor, state_i: torch.Tensor) -> torch.Tensor:
        x = torch.cat([state_0, state_i], dim=-1)
        return self.net(x)
...
class ImageEditingModel(nn.Module):
    ...
    def forward(
        ...
        cond = self.action_mlp(state_0, state_i)
