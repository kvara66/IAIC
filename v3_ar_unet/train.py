"""
Non-AR training: predict frame_i from frame_0 + [state_0, state_i, t/15].

Bigger UNet (ch=[256,512,1024]) + explicit timestep conditioning.
...
def parse_args():
    ...
    p.add_argument("--output-dir",        default="runs/v4_noar_big_t")
...
def train_step(
    ...
    def step_fn(f0, s0, si, gt_i, t_n):
        with torch.cuda.amp.autocast(enabled=amp):
            pred, _ = model(f0, s0, si, t_n)
    ...
    for t in range(1, SEQ_LEN):
        ...
        pred, d_loss = checkpoint(
            step_fn,
            frame_0, state_0, states[:, t], frames[:, t], t_norm,
            use_reentrant=False,
        )
