"""
Non-AR training: predict frame_i from frame_0 + absolute action pair [state_0, state_i].

No teacher forcing, no AR loop, no error compounding.
- train_step: 15 independent predictions from frame_0, full DINO+R3D+Action loss
- val_step:   same non-AR generation + full metric
"""
...
def parse_args():
    ...
    p.add_argument("--output-dir",        default="runs/v4_noar_big")
...
def train_step(
    ...
    frame_0 = frames[:, 0]   # (B, 3, H, W) — fixed reference, never updated
    state_0 = states[:, 0]   # (B, 6)        — initial joint state

    def step_fn(f0, s0, si, gt_i):
        with torch.cuda.amp.autocast(enabled=amp):
            pred, _ = model(f0, s0, si)
    ...
    # 15 independent predictions — no AR, no curr_frame update
    for t in range(1, SEQ_LEN):
        pred, d_loss = checkpoint(
            step_fn,
            frame_0, state_0, states[:, t], frames[:, t],
            use_reentrant=False,
        )
...
def val_step(model, frames, states, dino_fn, r3d_fn, action_fn, amp):
    ...
    with torch.no_grad():
        ...
        for t in range(1, SEQ_LEN):
            with torch.cuda.amp.autocast(enabled=amp):
                pred, _ = model(frame_0, state_0, states[:, t])  # always from frame_0
